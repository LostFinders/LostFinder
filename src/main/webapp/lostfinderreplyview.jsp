...
<html>
	...
	<section>
		<table id="board-item" border=1 align=center>
			...
			<tr>
				<td id="footer" align=left><!-- 여기 수정해서 작성버튼을 오른쪽으로 -->
						...
						<button id="create" class="btn btn-outline-success" type="button" style="float: right;" aria-label="Search">작성</button> <!-- 얘가 위치를 수정해야할 작성버튼 -->
				</td>
			</tr>
		</table>
			...
			<c:if test="${lostfinderreplyData ne null}">.
				<c:forEach var="i" items="${lostfinderreplyData}" begin="0" end="${fn:length(lostfinderreplyData)}" step="1">
					<tr>
						...
						<td><button class="update" value="${i.reply_uuid }">수정</button></td>
						<td><button class="delete" value="${i.reply_uuid }">삭제</button></td>
					</tr>
				</c:forEach>
			</c:if>
		</table>
		...
		<script src="lostfinderreplycreate.js"></script>
		<script src="lostfinderreplyview.js"></script>
	</body>
</html> 